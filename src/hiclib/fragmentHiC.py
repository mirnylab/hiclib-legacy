...
class HiCdataset(object):
    ...
    def evaluate(self, expression, internalVariables, externalVariables={},
        ...
        for start, end in bins:
            ...
            if type(outVariable) == str:
                self.h5dict.get_dataset(outVariable)[start:
                                                     end] = variables[outVariable]
                ...
                raise ValueError("Please provide str or (str,value) for out variable")
    ...
    def parseInputData(self, dictLike, zeroBaseChrom=True, enzymeToFillRsites=None, removeSS=False):
        ...
        if noRsites == True:  # We have to fill rsites ousrlves. Let's see what enzyme to use!
            if (enzymeToFillRsites is None) and (self.genome.hasEnzyme() == False):
        ...
        mask = self.evaluate("a = (fragids1 != fragids2)", ["fragids1",
                                                            "fragids2"])  # Discard dangling ends and self-circles
        maskLen, noSameFrag = len(mask), mask.sum()
        mask *= ((self.chrms1 < self.chromosomeCount) * (self.chrms2 <
                                                         self.chromosomeCount))  # Discard unused chromosomes
    ...
    def updateGenome(self, newGenome, removeSSreads="trans", oldGenome="current"):
        ...
        if removeSSreads.lower() == "none":
            ...
            self.maskFilter((self.chrms1 >= 0) * (self.chrms1 <
                                                  newN) + (self.chrms2 >= 0) * (self.chrms2 < newN))
    ...
    def fragmentSum(self, fragments=None, strands="both"):
        ...
        if strands == 2:
            return sumByArray(fasterBooleanIndexing(self.fragids2, self.DS), fragments)
...
class HiCStatistics(HiCdataset):
    ...
    def plotScaling(self, fragids1=None, fragids2=None,  # IDs of fragments for which to plot scaling.
        ...
        **kwargs :  optional
        ...
        regionID = np.zeros(len(self.chrms1), np.int16) - \
            1  # Region to which a read belongs
        chr1 = self.chrms1
        chr2 = self.chrms2
        pos1 = self.mids1
        pos2 = self.mids2
        ...
        if excludeNeighbors is not None:
            if enzyme is None:
                raise ValueError("Please specify enzyme if you're excluding Neighbors")
        ...
        for regionNumber, region in enumerate(regions):
            ...
            for lenmin, lenmax, index in map(None, lenmins, lenmaxs, range(len(lenmins))):
                ...
                if useWeights == False:
                    ...
                    curcount = np.sum(w1 * np.abs(sw2[val1] - sw2[val2]))  # (difference in cumsum of weights) * my weight
                ...
                if excludeNeighbors is not None:  # now modifying expected count because of excluded fragments
                    if useWeights == False:
                    ...
                    if (ignore >= curcount) and (ignore != 0):
                        if ignore < curcount * 1.0001:
                            ...
                            print "error found", "lenmin:", lenmin, "  curcount:", curcount, "  ignore:", ignore
