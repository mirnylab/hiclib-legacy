...
import sys 
import gzip
#sys.path.insert(0, "/home/magus/HiC2011/libraries/hiclib/src")
import logging
...
genome_db = genome.Genome("../data/mm9", readChrms=["#", "X", "Y", "M"], cacheDir="tmpDir")
...
seqSkipStart = 2  # skip first 2 bp of the read, if you want
...
for i in  iterList:
    ...
    if mode == "sra":
        ...
        counters = mapping.splitSingleFastq(firstSide, os.path.join(fastqFolder, expName + "_chunk{0:04d}_fhtagn_side1.fastq.gz"), chunkSize)
    ...
    def doOne(inData):
        ...
        mapping.parse_sam(sam_basename1=sf1, sam_basename2=sf2,
            out_dict=mapped_reads, genome_db=genome_db, save_seqs=False, maxReads=chunkSize*3, IDLen=50)
