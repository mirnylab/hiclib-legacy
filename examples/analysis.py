...
def saddlePlot():
    ...
    def reorder(data, array=PC):
        ...
        ndata = data[inds, :]
        return ndata[:, inds]
    toplot = (mirnylib.numutils.coarsegrain(
        reorder(Tanay.dataDict["GM-all"]), 60))
    toplot /= toplot.mean()
...
def plotScalingsByDomains(build=True):
    ...
    TR = HiCStatistics(
        "working/working.frag", genome, maximumMoleculeLength=500)
    TR.load(fragfile)
    TR.saveHeatmap("working/workingHeatmap", resolution=resolution)
        #Simply load corrected dataset and save it as a heatmap
    TR.maskFilter((TR.chrms1 == TR.chrms2) * (TR.strands1 == TR.strands2)
                  )  # making it easier to calculate scaling

    HM = hiclib.binnedData.binnedDataAnalysis(
        resolution, genome)  # load heatmap
    ...
    domains = hiclib.domainSearch.exponentialDomains(
        data)  # most advanced domain finder

    cr = scipy.stats.spearmanr(domains, HM.trackDict["GC"])
        #be sure that positive domains are always open
    print "correlation with GC is %lf" % cr[0]
    if cr[0] < 0:
        domains = -domains  # inverting domains if spearmanr(GC,dom) < 0

    domainIndex = numpy.zeros(len(domains), int)
        #Building 2 groups of bins: first third and last third
    ...
    chromosomeStarts = numpy.array(
        HM.genome.chrmStartsBinCont)  # chromosome starts

    fragments = TR.ufragments  # Now filtering fragments
    pos = fragments % TR.fragIDmult
    chrom = fragments / TR.fragIDmult
    mask1 = domainIndexRescaled[(pos / resolution) + chromosomeStarts[
        chrom]] == 1
    mask2 = domainIndexRescaled[(pos / resolution) + chromosomeStarts[
        chrom]] == -1  # fragment masks for open/closed regions
