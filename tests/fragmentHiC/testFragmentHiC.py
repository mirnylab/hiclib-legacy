...
if not os.path.exists(genomeFolder):
    ...
    if not os.path.exists(genomeFolder):
        raise Exception("Please provide hg18 Genome folder in the code or as a first argument")
    ...
    if create == True:
        for onename in filename[0]:
            ...
            if not os.path.exists(onename):
                raise Exception("path not found: %s" % onename)
